    ...
    <div v-if="!formDone">
      ...
      <form action="#" class="form" @submit.prevent="submitHandler">
        ...
        <div class="form__row">
          ...
          <div class="form__group">
            <label for="name" :class="{ comlite: !$v.form.name.$invalid }"
              >Имя*</label
            >
        ...
        <div class="form__row">
          ...
          <div class="form__group">
            ...
            <select v-model.trim="form.gender" id="gender">
              <option v-for="opt in genderData" :key="opt.id" :value="opt.id">{{
                opt.display
              }}</option>
            </select>
          </div>
            ...
            <span
              ...
              v-if="
                ($v.form.phone.$dirty && !$v.form.phone.required) ||
                  ($v.form.phone.$dirty && !$v.form.phone.numeric) ||
                  ($v.form.phone.$dirty && !$v.form.phone.between)
              "
              >Введите телефон в формате 7XXXXXXXXXX</span
            >
          </div>
        </div>
        <div class="form__row">
          <div class="form__group">
            ...
            <select
              ...
              id="clientGroup"
              multiple
              :size="clientsGroupData.length"
            >
              <option
                v-for="opt in clientsGroupData"
                :key="opt.id"
                :value="opt.id"
                >{{ opt.display }}</option
              >
        ...
        <div class="form__row">
          <div class="form__group form__group-small">
            ...
            <input
              type="text"
              v-model.number.trim="form.zipCode"
              id="zipCode"
            />
          </div>
          <div class="form__group">
...
.form {
  ...
  .form__error {
    font-size: 0.8rem;
    color: salmon;
    animation: opacity 0.3s 1 ease-in-out;
    position: absolute;
    top: calc(100% - 20px);
    left: 10px;
    margin-left: 5px;
